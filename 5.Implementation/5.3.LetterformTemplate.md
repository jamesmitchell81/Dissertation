...
As a highlevel representation of the algothim the follow presents the key steps as provided by the full algorithm in figure .. TODO figure.

1. Iterate of the binary image until a black pixel is found.
2. Store the start pixel for later reference to be involved in the stopping critrea.
3. Set the start pixel as centre of moore neighbourhood
4. Go back to the pixel previous to the start pixel was found.
5. Move in a clockwise(or anticlockwise) direction around the centre pixel of the neighbourhood concidering each pixel.
6. Should the concidered pixel be the start pixel, concider exitting the procedure.
7. Where the concidered pixel is black, make this the centre pixel and set the concidered pixel to the previously concidered pixel.
8. repeat 4.
...
![Working of Moore’s Neighbor tracing algorithm. (Samet and Hancer, 2012)](../-img/5.3.working-mn.png)
...
![Figure: Starting letterform subject (thresholded to remove alising)](../-img/5.3.trace-start.png)

![Figure: Thinned subject](../-img/5.3.trace-thin.png)
